...
class Assignments_List {
	...
	public function column_student( $item ) {
		...
		echo '<p class="tutor-fs-6 tutor-fw-medium  tutor-color-black">' . $item->comment_author . '</p>';
	}
	...
	public function column_action_evaluate( $item, $post_id ) {
		...
		echo "<a href='" . admin_url( "admin.php?page=tutor-assignments&view_assignment=$item->comment_ID&post-id=$post_id" ) . "'>
		<button class='tutor-btn tutor-btn-wordpress-outline tutor-btn-sm'>" . $button_text . '</button></a>';
	}
	...
	public function column_action_delete( $item ) {
		echo '<a class="tutor-color-muted" data-assignment_id="' . esc_attr( $item->comment_ID ) . '" href="#" data-tutor-modal-target="tutor-assignment-modal-' . esc_attr( $item->comment_ID ) . '">
		<button class="tutor-btn tutor-btn-disable-outline tutor-no-hover tutor-btn-sm">' . _x( ' Delete', 'assignment delete', 'tutor-pro' ) . '</button></a>';
	}
