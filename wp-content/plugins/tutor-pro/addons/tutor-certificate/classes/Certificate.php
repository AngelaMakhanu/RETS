...
class Certificate {
	...
	public function download_btn_in_archive($html, $course_id) {
		...
		if($is_completed_course) {
			if($this->has_course_certificate_template($course_id) && $certificate_url = $this->get_certificate($course_id)) {
				$html = '<a href="'.$certificate_url.'" class="tutor-btn tutor-btn-tertiary tutor-is-outline tutor-btn-md tutor-btn-full tutor-pr-0 tutor-pl-0">
					'.__('Download Certificate', 'tutor-pro').'
				</a>';
			} else {
				$html = '<button disabled="disabled" class="tutor-btn tutor-btn-disable-outline tutor-btn-outline-fd tutor-btn-md tutor-btn-full tutor-pr-0 tutor-pl-0">
					'.__('Download Certificate', 'tutor-pro').'
				</button>';
			}
		}
	...
	public function show_course_has_certificate( $meta, $course_id ) {
		if ( $this->has_course_certificate_template($course_id) ) {
			$meta[] = array(
				'icon_class' => 'tutor-icon-certificate-l',
