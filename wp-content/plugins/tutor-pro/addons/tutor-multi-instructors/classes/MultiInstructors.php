...
class MultiInstructors{
	...
	public function __construct() {
		...
		if (tutor_utils()->get_option('enable_course_marketplace')) {
			...
			add_action( 'add_meta_boxes', function() {
				add_meta_box( 'tutor-instructors', __( 'Instructors', 'tutor' ), array( $this, 'instructors_metabox' ), tutor()->course_post_type );
			} );
	...
	public function tutor_course_instructor_search(){
		...
		if (is_array($instructors) && count($instructors)){
			foreach ($instructors as $instructor){
				$search_result .= '<div class="tutor-instructor-search-single" data-user_id="'.$instructor->ID.'">
					<img src="'. get_avatar_url($instructor->ID) . '"/>
					<span class="instructor-name tutor-text-nowrap">
						'.$instructor->display_name.'
						<span class="tutor-d-block">'.$instructor->user_email.'</span>
					</span>
					<i class="tutor-icon-plus-bold-filled tutor-shortlist-instructor"></i>
				</div>';
			}
		} else {
		...
		$shortlisted_html = '<div class="tutor-fs-6 tutor-fw-medium tutor-color-black-70 tutor-mb-8">'.__('New Instructors', 'tutor-pro').'</div>
