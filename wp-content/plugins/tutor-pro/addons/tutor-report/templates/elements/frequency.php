			...
			<?php foreach ( $frequencies as $key => $frequency ) : ?>
				<div class="tutor-dropdown-select-option" data-key="<?php echo esc_attr( $key ); ?>">
					...
					<label for="select-item-1">
						<div class="tutor-fs-7 tutor-color-black-70 tutor-admin-report-frequency" data-key="<?php echo esc_attr( $key ); ?>">
	...
	<div class="tutor-dropdown-select-selected">
		<div class="">
			...
			<?php elseif ( 'last30days' === $current_frequency ) : ?>
				...
				<div class="tutor-fs-7 tutor-color-muted">
					<?php echo esc_html( date_i18n( 'M d', strtotime( gmdate( 'Y-m-d' ) ) ) . ' - ' . date_i18n( 'M d, Y', strtotime( date_format( $add_30_days, 'Y-m-d' ) ) ) ); ?>
				</div>
			<?php elseif ( 'last90days' === $current_frequency ) : ?>
				...
				<div class="tutor-fs-7 tutor-color-muted">
					<?php echo esc_html( date_i18n( 'M d', strtotime( gmdate( 'Y-m-d' ) ) ) . ' - ' . date_i18n( 'M d, Y', strtotime( date_format( $add_90_days, 'Y-m-d' ) ) ) ); ?>
				</div>
			<?php elseif ( 'last365days' === $current_frequency ) : ?>
				...
				<div class="tutor-fs-7 tutor-color-muted">
					<?php echo esc_html( date_i18n( 'M d', strtotime( gmdate( 'Y-m-d' ) ) ) . ' - ' . date_i18n( 'M d, Y', strtotime( date_format( $add_365_days, 'Y-m-d' ) ) ) ); ?>
				</div>
			<?php else : ?>
				<div class="tutor-fs-6 tutor-fw-medium  tutor-color-black">
					<?php echo esc_html( $frequencies[ $current_frequency ] ); ?>
