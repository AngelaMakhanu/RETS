...
if ( is_admin() ) {
	...
	?>
		<form action="" method="get" id="tutor-zoom-search-filter-form" class="tutor-wp-dashboard-filter tutor-mb-24">
			<div class="tutor-wp-dashboard-filter-items tutor-d-flex tutor-flex-xl-nowrap tutor-flex-wrap tutor-justify-content-between">
				<div class="tutor-wp-dashboard-filter-item tutor-col">
					...
					<div class="tutor-input-group tutor-form-control-has-icon tutor-form-control-sm">
						...
						<input name="search" type="search" class="tutor-form-control tutor-form-control-fd" value="<?php echo $_search; ?>" autocomplete="off" placeholder="<?php _e( 'Search meeting', 'tutor-pro' ); ?>">
					</div>
				</div>
				<div class="tutor-wp-dashboard-filter-item tutor-col">
					<label class="tutor-form-label">
					...
					<select name="course-id" class="tutor-zoom-course tutor-form-select tutor-form-select-fd tutor-form-control-sm" style="width:100%; max-width:100%;">
...
<div class="<?php echo is_admin() ? 'tutor-mt-24' : ''; ?>">
	...
	if ( ! empty( $meetings ) ) {
		...
		$name_sort = http_build_query( array_merge( $_GET, ( $orderby == 'post_title' ? $sort_order : array() ), array( 'orderby' => 'post_title' ) ) );
			...
			$time_icon = $orderby == 'datetime' ? ( strtolower( $order ) == 'asc' ? 'tutor-icon-order-up-filled' : 'tutor-icon-order-down-filled' ) : 'tutor-icon-order-up-filled';
			$name_icon = $orderby == 'post_title' ? ( strtolower( $order ) == 'asc' ? 'tutor-icon-order-up-filled' : 'tutor-icon-order-down-filled' ) : 'tutor-icon-order-up-filled';
