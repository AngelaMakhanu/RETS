...
class Zoom {
		...
		<div data-course_content_id="<?php echo $meeting->ID; ?>" id="<?php echo $row_id; ?>" class="course-content-item tutor-zoom-meeting-item tutor-zoom-meeting-<?php echo $meeting->ID; ?>">
			<div class="tutor-course-content-top">
				...
				<div class="tutor-course-content-top-right-action">
					<?php if($topic->ID>0): ?>
						<a href="javascript:;" class="tutor-zoom-meeting-modal-open-btn" data-tutor-modal-target="tutor-zoom-modal-cb-<?php echo $meeting->ID; ?>">
							<span class="tutor-color-muted tutor-icon-edit-filled tutor-fs-4 tutor-lh-1"></span>
						</a>
					<?php endif; ?>

					<a href="javascript:;" data-tutor-modal-target="<?php echo $id_string_delete; ?>">
						<!-- <i class="tutor-icon-garbage-line"></i> -->
						<span class="tutor-icon-garbage-line tutor-color-muted tutor-icon-delete-stroke-filled tutor-fs-4 tutor-lh-1"></span>
					</a>
				</div>
			</div>
			<?php
				...
				require dirname( __DIR__ ) . '/views/template/delete-modal.php';
			?>
		</div>
		<?php
	}
	...
	public function add_meeting_option_in_topic( $topic_id, $course_id ) {
		...
		if ( ! empty( $api_key ) && ! empty( $api_secret ) ) {
			?>
			<button class="tutor-btn tutor-is-outline tutor-is-sm" data-tutor-modal-target="<?php echo $new_modal_; ?>">
				<i class="tutor-icon-zoom" style="font-size: 20px; margin-right: 3px;"></i>
	...
	public function add_zoom_menu( $nav_items ) {
		...
		$new_items 	= array(
			'zoom' =>  array( 'title' => __( 'Zoom', 'tutor-pro' ), 'auth_cap' => tutor()->instructor_role, 'icon' => 'tutor-icon-zoom' )
		);
