		...
		foreach($rules as $rule) {
			if($rule->action == 'views'){
				...
				if($rule->type == 'specific-lesson'){
					...
					$completed_rule = wpc_get_module_percent_done($rule->module_id, $user_id, 1) >= $rule->percent ? true : false;
				} elseif($rule->type == 'any-module'){
					...
					if(!empty($modules)) {
						foreach($modules as $module) {
							if( wpc_get_module_percent_done($module->ID, $user_id, 1) >= $rule->percent ){
								$times++;
							}
						}
					}
		...
		if( $post_type == 'lesson' || $post_type == 'wpc-quiz'){
			...
			if($query->have_posts()){
				...
				if($award_email == true  && $send_email != 'false') {
					...
					$address = '<p><br><b>' . esc_html($name) . '</b><br>' . esc_html($unit) . ' ' . esc_html($address) . '<br>';
					$address .= esc_html($city) . ' ' . esc_html($state) . ' ' . esc_html($zip) . '<br>';
					$address .= esc_html($country) . '</p>';
					...
					foreach($to_award as $email){
						if($email['type'] == 'wpc-email'){
							if(!empty($user->user_email)){
								...
								$unsub = '<p class="unsub"><a href="' . esc_url($home_url) . '?wpc-user-id=' . (int) $user_id . '&wpc-unsub=true">' . esc_html( __('Click here', 'wpc-emails') ) . '</a> ' . 'to unsubscribe from fututre emails from ' . esc_url($home_url) . '</p>';
				...
				if($award_badge == true || $award_certificate == true){
					$text = esc_html( __("You've Received One or More Awards", 'wp-courses') );
					$data .= '<div class="wpc-award-lightbox-wrapper"><div class="wpc-award-lightbox"><h4 class="wpc-lightbox-header">' . wp_kses($text, 'post') . '</h4><div class="wpc-lightbox-close"><i class="fa fa-times"></i></div><div class="wpc-award-lightbox-content">';
					
					if($award_badge == true ){
						$data .= '<h4>' . esc_html( __("Badges", 'wp-courses') ) . '<h4>';
					...
					if($award_certificate == true){
						$data .= '<h4>' . esc_html( __("Certificates", 'wp-courses') ) . '<h4>';
						foreach($to_award as $award){
							if($award['type'] == 'wpc-certificate'){
								$details = '<a href="#" class="wpc-show-requirements wpc-button" data-id="' . $award['id'] . '">' . esc_html__('View Requirements') . '</a>';
								$link = '<a href="' . get_the_permalink($award['id']) . '" class="wpc-button">' . esc_html__('View Certificate') . '</a>';
								$data .= '<div class="wpc-certificate-wrapper"><h3 class="wpc-h3 wpc-certificate-title">' . get_the_title($award['id']) . '</h3>' . $details . $link . '</div>';
							}
						}
					}
