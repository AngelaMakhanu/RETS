...
function wpc_connected_course_to_lesson_meta_box_callback( $post ) { 
	...
	<?php echo '<br><a style="margin: 5px 0 0;" href="' . admin_url() . 'post-new.php?post_type=course" class="button">' . esc_html__('Add New Course', 'wp-courses') . '</a>';
}
...
function wpc_lesson_restriction_meta_box_callback( $post ) {
	...
	$restriction = sanitize_title_with_dashes( get_post_meta( $post->ID, 'wpc-lesson-restriction', true ) );
