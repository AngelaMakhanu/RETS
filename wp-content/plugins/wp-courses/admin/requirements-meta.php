...
function wpc_requirements_meta_box_callback( $post ) {
		...
		<div class="wpc-admin-toolbar wpc-metabox-toolbar">
			<button id="wpc-add-new-requirement" class="button" type="button"><?php esc_html_e('Add Requirement', 'wp-courses'); ?></button>
		</div>
			...
			<?php
				...
				if(!empty($results)){
					foreach($results as $result){ ?>
						<div class="wpc-requirement" data-requirement-id="<?php echo (int) $result->id ?>">

							<label><?php esc_html_e('When Someone', 'wp-courses'); ?>: </label>
							<select name="wpc-requirement-action" class="wpc-requirement-action">
								 <option value="views" <?php echo $result->action == 'views' ? ' selected' : '' ?>><?php esc_html_e('Views', 'wp-courses'); ?></option>
								 <option value="completes" <?php echo $result->action == 'completes' ? ' selected' : '' ?>><?php esc_html_e('Completes', 'wp-courses'); ?></option>
								 <option value="scores" <?php echo $result->action == 'scores' ? ' selected' : '' ?>><?php esc_html_e('Scores', 'wp-courses'); ?></option>
							 </select>

							 <select name="wpc-requirement-type" class="wpc-requirement-type">
								 <option value="any-course" <?php echo $result->type == 'any-course' ? ' selected' : '' ?> class="<?php echo $result->action == 'scores' ? 'wpc-hide' : ''; ?>"><?php esc_html_e('Any Course', 'wp-courses'); ?></option>
								 <option value="specific-course" <?php echo $result->type == 'specific-course' ? ' selected' : '' ?> class="<?php echo $result->action == 'scores' ? 'wpc-hide' : ''; ?>"><?php esc_html_e('A Specific Course', 'wp-courses'); ?></option>
								 <option value="any-lesson" <?php echo $result->type == 'any-lesson' ? ' selected' : '' ?> class="<?php echo $result->action == 'scores' ? 'wpc-hide' : ''; ?>"><?php esc_html_e('Any Lesson', 'wp-courses'); ?></option>
								 <option value="specific-lesson" <?php echo $result->type == 'specific-lesson' ? ' selected' : '' ?> class="<?php echo $result->action == 'scores' ? 'wpc-hide' : ''; ?>"><?php esc_html_e('A Specific Lesson', 'wp-courses'); ?></option>
								 <option value="any-module" <?php echo $result->type == 'any-module' ? ' selected' : '' ?> class="<?php echo $result->action == 'scores' ? 'wpc-hide' : ''; ?>"><?php esc_html_e('Any Module', 'wp-courses'); ?></option>
								 <option value="specific-module" <?php echo $result->type == 'specific-module' ? ' selected' : '' ?> class="<?php echo $result->action == 'scores' ? 'wpc-hide' : ''; ?>"><?php esc_html_e('A Specific Module', 'wp-courses'); ?></option>
								 <option value="any-quiz" <?php echo $result->type == 'any-quiz' ? ' selected' : '' ?>><?php esc_html_e('Any Quiz', 'wp-courses'); ?></option>
								 <option value="specific-quiz" <?php echo $result->type == 'specific-quiz' ? ' selected' : '' ?>><?php esc_html_e('A Specific Quiz', 'wp-courses'); ?></option>
							 </select>
							...
							<label class="wpc-percent-label <?php echo $class; ?>"><?php esc_html_e('Percent', 'wp-courses'); ?>: </label>
							<input type="number" min="0" max="100" value="<?php echo (int) $result->percent; ?>" class="wpc-percent <?php echo $class; ?>"/>
							...
							<label class="wpc-times-label <?php echo $class; ?>"><?php esc_html_e('Times', 'wp-courses'); ?>: </label><input name="wpc-times" type="number" min="1" value="<?php echo (int) $result->times; ?>" class="wpc-requirement-times <?php echo $class; ?>"/>

							<br>
							
							<button class="wpc-delete-requirement button" type="button" data-requirement-id="<?php echo (int) $result->id ?>"><?php esc_html_e('Delete Requirement', 'wp-courses'); ?></button>
