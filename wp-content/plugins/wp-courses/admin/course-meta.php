...
function wpc_course_lessons_meta_box_callback( $post ) {
	echo '<button type="button" id="wpc-add-module" class="button" style="margin-top:10px;">Add Module</button>';
	wpc_admin_lesson_list($post->ID);
}
...
function wpc_course_meta_box_callback( $post ) {
	...
	$none_selected = in_array(-1, $teachers) ? 'selected' :'';

	echo '<select id="teacher-select" name="teacher-selection[]" class="wpc-teacher-multiselect" multiple>';
	echo '<option value="-1" ' . $none_selected  . '>' . esc_html__('None', 'wp-courses') . '</option>';
	while($query->have_posts()){
		$query->the_post();
		$teacher_id = get_the_ID();
		foreach($teachers as $teacher){
			$selected = $teacher == $teacher_id ? 'selected="selected"' : false;
			if($selected != false) {
				break;
			}
		}
		echo '<option value="' . (int) $teacher_id . '"' . $selected . '>' . get_the_title() . '</option>';
	}
	wp_reset_postdata();
	echo '</select>';
	$admin_url = admin_url();
	echo '<br><a href="' . esc_url($admin_url) . 'post-new.php?post_type=teacher" class="page-title-action add-new button" style="margin-top: 8px">' . esc_html( __('Add New', 'wp-courses') ) . '</a>';
	...
	?>
	<br><label><?php esc_html_e('Course Category', 'wp-courses'); ?>:</label>
